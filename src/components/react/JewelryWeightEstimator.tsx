  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
      <div className="p-5 md:p-10">
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-3 mb-8 md:mb-10">
          {(Object.entries(JEWELRY_DATA) as [JewelryType, typeof JEWELRY_DATA[JewelryType]][]).map(([key, data]) => (
            <button
              ...
              className={`
                flex items-center gap-1.5 md:gap-2 px-3 py-2 md:px-5 md:py-2.5 rounded-md border font-semibold text-xs md:text-sm transition-colors duration-150
                ${activeType === key 
                  ? 'bg-ink-950 border-ink-950 text-white [&>svg]:text-gold-400' 
        ...
        <div className="flex justify-center gap-2 mb-8 md:mb-10">
          {COMMON_PURITIES.map((code) => (
            <button
              ...
              className={`px-4 py-2 rounded-md border font-semibold text-sm num transition-colors duration-150
                ${selectedPurity === code
                  ? 'bg-ink-950 border-ink-950 text-white [&>svg]:text-gold-400'
                  : 'bg-white border-gray-300 text-gray-700 hover:border-ink-400'
                }`}
          ...
          {currentData.sizes.map((size, index) => {
            ...
            return (
              <button
                ...
                className={`
                  relative p-3 md:p-6 rounded-lg border text-center md:text-left transition-colors duration-150 group flex flex-col items-center md:block
                  ${isSelected 
                    ? 'border-ink-950 bg-white ring-1 ring-ink-950 shadow-md' 
                    : 'border-gray-200 bg-white hover:border-ink-400'
                  }
                `}
              >
                <div className="flex justify-center md:justify-between items-start w-full mb-1 md:mb-3">
                  ...
                  {isSelected && (
                    <span className="hidden md:flex h-2.5 w-2.5 rounded-full bg-gold-400"></span>
                  )}
                </div>
                {/* Kuvaus näkyy vain desktopilla */}
                <p className="text-sm text-gray-500 mb-6 min-h-[40px] hidden md:block">{size.desc}</p>
                
                <div className="text-xl md:text-3xl font-semibold text-gray-900 tracking-[-0.02em] num mt-1 md:mt-0">
                  ~{size.weight}<span className="text-sm md:text-lg font-medium text-gray-500 ml-0.5">g</span>
                </div>
                {/* Euroarvio suoraan korttiin — kaikki koot vertailtavissa yhdellä silmäyksellä */}
                {spotPrice > 0 && (
                  <div className={`text-xs md:text-sm font-semibold mt-1 num ${isSelected ? 'text-gold-700' : 'text-gray-500'}`}>
        ...
        {selectedSize !== null && (
          <div className="bg-ink-950 rounded-xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 md:gap-8 text-white relative overflow-hidden ring-1 ring-ink-800 shadow-lg">
            <span className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-gold-400/60 to-transparent pointer-events-none" aria-hidden="true"></span>
            
            <div className="relative z-10 text-center md:text-left w-full">
              <span className="inline-block text-gold-400 text-[11px] font-semibold uppercase tracking-[0.18em] mb-2 md:mb-3">
                Arvioitu myyntihinta
              </span>
              <div className="text-4xl md:text-6xl font-semibold tracking-[-0.035em] num mb-2">
                {calculateEstimate(currentData.sizes[selectedSize].weight)}
              </div>
              <p className="text-gray-300 text-xs md:text-sm num">
            ...
            <div className="relative z-10 shrink-0 w-full md:w-auto">
              <button
                ...
                className="w-full md:w-auto bg-gold-400 text-ink-950 px-6 py-3.5 rounded-lg font-semibold border border-gold-300/60 hover:bg-gold-300 transition-colors flex items-center justify-center gap-2"
              >
                <Circle size={16} className="text-ink-950 fill-current" />
                Laske tarkka hinta
              </button>
            </div>
          </div>
